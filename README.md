...
5. Independently of the layer thresholds, each actor projection passes a complete-input hard-headroom gate. With production settings, the gate counts the system prompt, tool schemas, rendered observation/reflection memory, and uncovered messages against the 200k limit. At hard pressure, memory work becomes blocking, retries one failed frozen observation, and aborts fail-closed with exact source preserved if safe headroom cannot be restored.
